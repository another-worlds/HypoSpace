...
def test_reference_kernel_match_score(tmp_path) -> None:
    config = DecoderConfig(runtime=RuntimeConfig(cache_dir=str(tmp_path)), top_k=2)
    api = HypoSpaceAPI(config=config)

    api.decode("demo", "layer_x", [0.9, 0.1, 0.0], version="0.1.0")
    run = api.decode_and_score(
        "candidate",
        "layer_x",
        [0.95, 0.11, 0.0],
        version="0.1.0",
        reference_kernel_id="demo-layer_x",
    )

    assert run.decode.match_score is not None
    assert 0.9 <= run.decode.match_score <= 1.0
