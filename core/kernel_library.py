...
class KernelLibrary:
    ...
    def similarity(self, left: Iterable[Feature], right: Iterable[Feature]) -> float:
        """Compute cosine similarity between sparse feature vectors keyed by source index."""
        lv = self._to_sparse_vector(left)
        rv = self._to_sparse_vector(right)
        if not lv or not rv:
            return 0.0

        keys = set(lv) | set(rv)
        dot = sum(lv.get(k, 0.0) * rv.get(k, 0.0) for k in keys)
        ln = sqrt(sum(v * v for v in lv.values()))
        rn = sqrt(sum(v * v for v in rv.values()))
        if ln == 0 or rn == 0:
            return 0.0
        return dot / (ln * rn)

    def match_latest(self, kernel_id: str, features: Iterable[Feature]) -> float:
        """Score similarity to the latest saved kernel."""
        latest = self.load_latest(kernel_id)
        return self.similarity(latest.features, features)
    ...
    @staticmethod
    def _to_sparse_vector(features: Iterable[Feature]) -> Dict[int, float]:
        vector: Dict[int, float] = {}
        for feature in features:
            vector[feature.source_index] = max(vector.get(feature.source_index, 0.0), feature.score)
        return vector
